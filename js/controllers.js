var ref = new Firebase("https://thindocsmuse000.firebaseio.com/");
var userAuth = JSON.parse(localStorage.getItem("firebase:session::thindocsmuse000"));
/**
 ...
 */
function dashboardFlotOne($scope, $http, $state) {
    
    var session = localStorage.getItem("firebase:session::thindocsmuse000");
...
function datatablesCtrl($scope,DTOptionsBuilder, $http){
     ...
     */
    $http.get("api/users/" + userAuth.uid).then(function(response){
        $scope.authedUser = response.data[0];
...
function LoginCtrl($scope, $http, $state, $firebaseAuth) {
    var session = localStorage.getItem("firebase:session::thindocsmuse000");
    ...
    $scope.login = function (user) {
        ...
        auth.$authWithPassword({
            ...
            userAuth = JSON.parse(localStorage.getItem("firebase:session::thindocsmuse000"));
            $state.go('dashboards.dashboard_1');
        },function(){
    ...
    $scope.hide_error_message = function(){
        $scope.error_msg = "";
    }
}

function RegisterCtrl($scope, $http, $state, $firebaseAuth) {
    ...
    $scope.add_new = function(user, AddNewForm) {
        ...
        auth.$createUser({
            ...
            object.password = btoa(user.password);
            object.uid = data.uid;
            usersRef.push(object);

            $http.post("api/user_registered" , object).then(function(response){
                $state.go('login');
            });
...
function profile2Ctrl($scope){
    var authObject = JSON.parse(localStorage.getItem("firebase:session::thindocsmuse000"));
    var authObjectData = {};
    $scope.user = {};
    $scope.loggedinUserObjectIndex = "";

    // Find current logged in user from Firebase Database
    var userRef = new Firebase("https://thindocsmuse000.firebaseio.com/users");
    userRef.on("value", function(snapshot) {
        for(i in snapshot.val()){
            var object = snapshot.val()[i];
            if(object.uid == authObject.uid){
                $scope.loggedinUserObjectIndex = i;
                angular.copy(object, $scope.user);
                $scope.user.uid = authObject.uid;
                authObjectData = object;
                break;        
            }
        }
        $scope.$apply({});
    ...
    $scope.update = function(user, ProfileForm){
        var reference = userRef.child($scope.loggedinUserObjectIndex);
        reference.update({
    ...
    $scope.updateEmail = function(user, ProfileForm){
        ref.changeEmail({
          oldEmail : authObjectData.email,
          newEmail : user.email,
          password : atob(authObjectData.password)
        }, function(error) {
          if (error === null) {
    ...
    $scope.updatePassword = function(user, ProfileForm){
        ref.changePassword({
          email       : authObjectData.email,
          oldPassword : atob(authObjectData.password),
          newPassword : user.newPassword
        }, function(error) {
          if (error === null) {
            var reference = userRef.child($scope.loggedinUserObjectIndex);
            reference.update({
                "password" : btoa(user.newPassword)
            });
          } else {
...
function forgotPwdCtrl($scope, $state){
    $state.success_msg = "";
    $state.error_msg = "";
    $scope.sendNewPasswordRequest = function(){
        ref.resetPassword({
          email : $scope.email
        }, function(error) {
          if (error === null) {
            $scope.success_msg = "Password reset email sent successfully";
          } else {
            $scope.error_msg = "Invalid User";
          }
          $scope.$apply({});
        });
    }
...
function clientsCtrl($scope, $http){
    $http.get("api/getProjects/" + userAuth.uid).then(function(response){
        $scope.projects = response.data;
    });

    $scope.changeProject = function($index){
        $scope.client = $index + 8;
    }
}
/**
 *
...
angular
    ...
    .controller('profile2Ctrl', profile2Ctrl)
